...
export default function AllPokemon({ back }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setMonList(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
      setLoading(false);
    })();
  }, []);
          ...
          {monList.length ? (
            monList.map((m) => (
              <div
                ...
                style={{ borderColor: getTypeColor(m.types[0]) }}
                onClick={() => setSelectedMon(m)}
              >
                <img src={m.sprite} alt={m.name} />
                <div className="poke-name">
                  {m.nickname || m.name.toUpperCase()}
                </div>
            ...
            <p className="empty-box">How did you get here without a pokemon?</p>
          )}
        </div>
      </div>
